...
def update_products_table(db_creds: DBCreds, **db_connection_kwargs):
    ...
    stg_products_aggregated = (
        sa.select([
            stg_products.columns["sku"],
            stg_products.columns["name"],
            sa.func.group_concat(stg_products.columns["description"].op("SEPARATOR")(sa.text("'; '"))).label("description"),
        ])
        .select_from(stg_products)
        .group_by(
            stg_products.columns["sku"],
            stg_products.columns["name"],
        )
        .alias("stg_products_aggregated")
    )
    ...
    insert_stmt = mysql.insert(dwh_products).from_select(
        ...
        (
            sa.select([
                ...
                stg_products_aggregated.columns["description"],
            ...
            .select_from(
                ...
                .join(
                    dwh_skus,
                    onclause=stg_products_aggregated.columns["sku"] == dwh_skus.columns["sku"],
                )
                .join(
                    dwh_names,
                    onclause=stg_products_aggregated.columns["name"] == dwh_names.columns["name"],
                )
            )
        ),
    )
...
def update_by_name_no_of_products_table(db_creds: DBCreds, **db_connection_kwargs):
    ...
    insert_stmt = sa.insert(dwh_by_name_no_of_products).from_select(
        ...
        sa.select([
            ...
            sa.func.count(sa.distinct(dwh_products.columns["sku_id"])).label("no_of_products"),
        ])
